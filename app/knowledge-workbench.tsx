...
const libraries: LibraryDefinition[] = [
  { id: "ai-work", mark: "AI", name: "AI 工作记录", subtitle: "会话、实验与决策", count: 286, unclassified: 14, collection: "kb_ai_work_v1", policy: "private · session-aware", description: "保存模型实验、提示词版本、工作日志与可复用决策；按项目、模型和时间组织。" },
  { id: "academic", mark: "AC", name: "学术资料", subtitle: "论文、书籍与引用", count: 1248, unclassified: 31, collection: "kb_academic_v1", policy: "research · citation-first", description: "强调作者、年份、DOI、版本和引用链，目录树按领域与研究问题逐步演化。" },
  { id: "production", mark: "PR", name: "生产文档", subtitle: "SOP、部署与故障", count: 719, unclassified: 8, collection: "kb_production_v1", policy: "team · recency-weighted", description: "强调版本有效期、责任人、环境与变更记录，过期内容不参与默认回答。" },
  { id: "notes", mark: "NT", name: "个人思维笔记", subtitle: "灵感、观察与草稿", count: 934, unclassified: 46, collection: "kb_notes_v1", policy: "owner-only · exploratory", description: "允许弱结构、交叉标签和持续生长的主题分支，不强迫每条笔记只有一个位置。" },
  { id: "association", mark: "∿", name: "关联知识库", subtitle: "跨库潜在关系", count: 4382, unclassified: 0, collection: "kb_association_v1", policy: "edge-only · cross-library", description: "只保存跨库关系、证据指针和置信度，不复制原文；用于发现支持、冲突、类比和反常组合。" },
];

const trees: Record<LibraryId, TreeNode[]> = {
  "ai-work": [
    { id: "ai-unclassified", label: "未归类", count: 14, hint: "新文件入口" },
    { id: "ai-projects", label: "项目", count: 168, children: [
      { id: "ai-rag", label: "Global RAG", count: 74, children: [{ id: "ai-rag-design", label: "设计决策", count: 21 }, { id: "ai-rag-debug", label: "调试记录", count: 38 }, { id: "ai-rag-prompts", label: "提示词版本", count: 15 }] },
      { id: "ai-agents", label: "Agent 实验", count: 56 },
      { id: "ai-automation", label: "自动化工作流", count: 38 },
    ] },
    { id: "ai-models", label: "模型评测", count: 71, children: [{ id: "ai-closed", label: "闭源 API", count: 29 }, { id: "ai-local", label: "本地模型", count: 42 }] },
    { id: "ai-decisions", label: "跨项目决策", count: 33 },
  ],
  academic: [
    { id: "ac-unclassified", label: "未归类", count: 31, hint: "等待路由卡" },
    { id: "ac-cs", label: "计算机科学", count: 624, children: [{ id: "ac-ir", label: "信息检索", count: 166, children: [{ id: "ac-hybrid", label: "混合检索", count: 63 }, { id: "ac-rerank", label: "重排序", count: 48 }] }, { id: "ac-llm", label: "语言模型", count: 284 }, { id: "ac-hci", label: "人机交互", count: 91 }] },
    { id: "ac-cog", label: "认知科学", count: 217 },
    { id: "ac-method", label: "研究方法", count: 376 },
  ],
  production: [
  ...
  notes: [
    { id: "nt-unclassified", label: "未归类", count: 46, hint: "允许长期停留" },
    { id: "nt-systems", label: "系统与复杂性", count: 214, children: [{ id: "nt-emergence", label: "涌现", count: 61 }, { id: "nt-feedback", label: "反馈回路", count: 44 }] },
    { id: "nt-making", label: "创造与方法", count: 308 },
    { id: "nt-observation", label: "观察记录", count: 219 },
    { id: "nt-seeds", label: "尚未成形的种子", count: 147 },
  ],
  association: [
    { id: "as-candidate", label: "候选关联", count: 186, hint: "等待人工确认" },
    { id: "as-support", label: "相互支持", count: 1384, children: [{ id: "as-support-direct", label: "直接证据", count: 946 }, { id: "as-support-analogy", label: "跨域类比", count: 438 }] },
    { id: "as-conflict", label: "冲突与例外", count: 729, children: [{ id: "as-version", label: "版本冲突", count: 246 }, { id: "as-counter", label: "反例", count: 311 }] },
    { id: "as-causal", label: "因果与条件", count: 1068 },
    { id: "as-counterintuitive", label: "反直觉假设", count: 1015, hint: "只作为待验证结论" },
  ],
};
...
export function KnowledgeWorkbench({ demoMode, gatewayUrl, apiKey, onNotice }: { demoMode: boolean; gatewayUrl: string; apiKey: string; onNotice: (message: string) => void }) {
  ...
  const activeLibrary = libraries.find((item) => item.id === activeId)!;
  const activeTree = trees[activeId];
  ...
  function chooseLibrary(id: LibraryId) {
    setActiveId(id);
    setSelectedNode(trees[id][0].id);
    setExpanded(new Set(trees[id].filter((item) => item.children).slice(0, 1).map((item) => item.id)));
  ...
  async function requestClassification() {
    ...
    setProposalState("running");
    try {
      await callGateway("/v1/taxonomy/proposals", { library_id: activeId, source_node: `${activeId}-unclassified`, mode: "preview", payload_mode: "routing_cards", taxonomy_scope: "affected_subtree" });
      setProposalState("preview");
      onNotice("AI 归类提案已生成，确认前不会移动文件");
    } catch (error) {
      setProposalState("idle");
  ...
  async function applyProposal() {
    try {
      await callGateway("/v1/taxonomy/proposals/demo-20260716/apply", { library_id: activeId, expected_taxonomy_version: 18 });
      setProposalState("idle");
      onNotice("已应用 11 项高置信度变更；3 项保留人工复核");
    } catch (error) {
  ...
  return <div className="page inner-page knowledge-page">
    ...
    <section className="kb-overview-strip" aria-label="知识库概览">
      <div><small>独立知识库</small><b>05</b><span>4 文档库 · 1 关联库</span></div>
      <div><small>未归类</small><b>99</b><span>仅存在预制大类中</span></div>
      <div><small>待复核提案</small><b>17</b><span>不会自动应用</span></div>
    ...
    <section className="kb-workbench">
      ...
      <section className="tree-workspace">
        ...
        {proposalState === "preview" && <div className="proposal-banner"><span className="proposal-pulse"/><div><b>归类提案 #20260716-018</b><p>11 项可直接归类 · 3 项低置信度留在“未归类” · 建议新建 2 个分支</p></div><button onClick={() => setProposalState("idle")}>关闭预审</button></div>}
      ...
      <aside className="detail-drawer">
        ...
        {proposalState === "preview" ? <>
          <div className="proposal-list">
            <div><span>MOVE</span><p><b>debug-session-42.json</b><small>未归类 → 项目 / Global RAG / 调试记录</small></p><em>0.96</em></div>
            <div><span>MOVE</span><p><b>research-notes-0715.md</b><small>未归类 → 模型评测 / 闭源 API</small></p><em>0.91</em></div>
            <div><span>BRANCH</span><p><b>评测基准 / 长上下文</b><small>建议新建分支 · 5 个文档命中</small></p><em>0.87</em></div>
            <div className="needs-review"><span>HOLD</span><p><b>paper-draft-v3.pdf</b><small>候选分支分差过小，保留未归类</small></p><em>0.54</em></div>
          </div>
          <div className="token-budget"><span>本次远程调用</span><b>≈ 3.8K tokens</b><small>14 张路由卡 + 相关子树，不上传全文</small></div>
          <div className="drawer-actions"><button onClick={() => setProposalState("idle")}>取消</button><button onClick={applyProposal}>应用高置信度变更</button></div>
        </> : <>
          <dl className="drawer-meta"><div><dt>对象数量</dt><dd>{node.count}</dd></div><div><dt>向量集合</dt><dd><code>{activeLibrary.collection}</code></dd></div><div><dt>隔离策略</dt><dd>{activeLibrary.policy}</dd></div><div><dt>目录版本</dt><dd>v18 · 可回滚</dd></div></dl>
          ...
          <section className="drawer-section relation-preview"><h3>最近活动</h3><p><span>18:42</span> taxonomy v18 已发布</p><p><span>18:31</span> 6 个文件完成索引</p><p><span>17:58</span> 1 项变更已撤销</p></section>
        </>}
      </aside>
    </section>
    ...
    {!isAssociation && <section className="ingest-dock">
      ...
      <label><span>预制大类</span><select value={activeId} onChange={(event) => chooseLibrary(event.target.value as LibraryId)}>{libraries.filter((item) => item.id !== "association").map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label>
