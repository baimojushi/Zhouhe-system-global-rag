...
set "WSL_USER=baimo"
set "SERVICE_NAME=llama_q4"
set "SERVICE_PORT=8000"
set "SOURCE_SH=%~dp0q4-server-persistent.sh"
set "LINUX_DIR=/home/baimo/.local/share/llama-start"
set "LINUX_SH=/home/baimo/.local/share/llama-start/q4-server-persistent.sh"
set "LOG_FILE=/home/baimo/q4-server.log"
set "MAX_ATTEMPTS=140"

echo ========================================
echo  Gemma 4 31B Q4_K_M persistent server
...
echo [3/6] 检查模型文件...
wsl.exe -d %DISTRO% -u %WSL_USER% --exec /usr/bin/test -s /home/baimo/models/gemma4-crack-Q4_K_M/gemma-4-31b-jang-crack-Q4_K_M.gguf >nul 2>&1
...
REM 清空旧日志
wsl.exe -d %DISTRO% -u %WSL_USER% --exec /usr/bin/rm -f /home/baimo/q4-server.log >nul 2>&1
...
wsl.exe -d %DISTRO% -u %WSL_USER% --exec /usr/bin/test -s ^
  /home/%WSL_USER%/.local/state/llama/%SERVICE_NAME%.server.pid >nul 2>&1
if errorlevel 1 (
    if !ATTEMPTS! leq 10 (
...
echo   /home/baimo/models/gemma4-crack-Q4_K_M/gemma-4-31b-jang-crack-Q4_K_M.gguf
if not "/home/baimo/models/gemma4-crack-Q4_K_M/gemma-4-31b-jang-crack-Q4_K_M.gguf"=="" echo.
pause
exit /b 1
